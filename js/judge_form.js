$(function() { 
    ...
    $.get('server/judge_form.php', dataGet, function(data) {
        ...
        for(z=0; z < formCount; z++){
            ...
            sum.innerHTML = '<h4>Grand Total (Sum of Design Project and Presentation Totals) <input type="text" class="grandtotal" readonly="readonly" name="sum'+z+'"/></h4>';
        ...
        for (j=0; j<formCount; j++){
            ...
            $('#title'+j).append("<h3> Title: "+data[j].title+"</h3>");
            $('#description'+j).append("<b>Description:</b><br> "+data[j].description+"");
            $('#session'+j).append("<p><b>Session:</b>  "+data[j].session+"");
            $('#category'+j).append("<p><b> Category:</b>  "+data[j].category+"");
	...
	if(!confirm("Are you sure you want to submit the forms?")){
        ...
        $.post(formURL, formData, function(data, textStatus, jqXHR) {
            ...
            $( "#projectEval" ).hide();
            if (data.trim() === "true") {
                $( "#result" ).empty().append('<div id="container"><div id="success">Successfully Added!</div><p>Thank you for your evaluations! We hope you enjoyed the conference. </div>'); 
            } else {
